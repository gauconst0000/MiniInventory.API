...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  // 🚀 MA THUẬT QUẢN LÝ BỘ NHỚ: Gom tất cả các luồng API vào đây để tiện quản lý
  ...
  // 🚀 LỜI GIẢI CỦA SẾP TÙNG: Dọn sạch rác và ngắt kết nối khi rời trang
  ngOnDestroy() {
    this.sub.unsubscribe(); // Cắt đứt mọi kết nối ngầm, chống rò rỉ bộ nhớ
    this.danhSachSanPham = []; // Xóa trắng dữ liệu cũ, không cho làm "màn che"
    this.hangSapHet = [];
    this.lichSuGiaoDich = [];
  }
  ...
  layDanhSachHangHoa() {
    ...
    // 🚀 Bọc luồng gọi API vào thùng chứa sub
    this.sub.add(
        ...
        this.http.delete(`${environment.apiUrl}/Products/${id}`, { headers: headers })
          .subscribe({
            next: () => {
              ...
              // 🚀 2. Xóa luôn nó khỏi bảng Cảnh báo bên trên
